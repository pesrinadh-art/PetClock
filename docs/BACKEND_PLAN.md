...
7. **Pet Walker Mode** — a scoped, time-boxed household role for a dog walker/sitter. Invite carries `role='walker'` + `member_expires_at` (enforcement is one added clause in `app.is_member` / the RLS policies on `pets`/`appointments`: walker role gets `logs` write + `pets`/`feed_times`/`medications` read, but no write). Client-side: walker's app opens straight to the pet(s) they're covering, with last pee/poo time + note surfaced first (reuse `PetCard`'s status tiles) and one-tap pee/poo/food/med logging — no schedule or appointment editing in the nav at all for this role. Logs write to the same `logs` table as any household member, so `trg_logs_after_insert` recomputes predictions and the realtime channel pushes the entry straight to the owner's open app, same as a co-owner's log; tag `logs.source` (or a new value alongside `notification_yes`) so the owner's Timeline can show "logged by dog walker."
